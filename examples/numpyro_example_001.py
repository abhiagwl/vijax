...
def model(data, dim):
    z = numpyro.sample('z', dist.LogNormal(jnp.zeros(dim), jnp.ones(dim)))
    numpyro.sample('obs', dist.Normal(z.sum(), 1), obs=data)

# Set the dimension of the model and generate some data
dim, data = 3, jnp.array([1.0, 2.0, 3.0])
...
vijax_model = NumpyroModelWrapper(model, jax.random.PRNGKey(0), (data, dim), {})
...
mcmc.run(jax.random.PRNGKey(1), data, dim)
...
plot_pairwise([mcmc_samples['z'], vi_samples_diag, vi_samples_gauss, vi_samples_realnvp], 
...
plot_violin([mcmc_samples['z'], vi_samples_diag, vi_samples_gauss, vi_samples_realnvp], 
            ['NUTS', 'Diagonal', 'Gaussian', 'RealNVP'])

# Calculate and print mean and variance for VI and MCMC samples
def calculate_statistics(vi_samples, mcmc_samples, vi_name):
    vi_mean, vi_variance = jnp.mean(vi_samples, axis=0), jnp.var(vi_samples, axis=0)
    mcmc_mean, mcmc_variance = jnp.mean(mcmc_samples['z'], axis=0), jnp.var(mcmc_samples['z'], axis=0)
...
calculate_statistics(vi_samples_diag, mcmc_samples, "Diagonal")
calculate_statistics(vi_samples_gauss, mcmc_samples, "Gaussian")
calculate_statistics(vi_samples_realnvp, mcmc_samples, "RealNVP")
